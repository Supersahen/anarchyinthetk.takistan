...
stats_human_side = {
	...
	if (_ch1 == ASCII_S && _ch2 == ASCII_U) exitWith { civilian };
...
stats_client_save = {
	if (isNil "RG_fnc_iSave") then {
		diag_log "ERROR: RG_fnc_iSave function not found!";
		exitWith {};
	};
	
	if (isNil "statsLoaded" || statsLoaded != 1) then {
		diag_log "ERROR: Stats not loaded yet!";
		statsLoaded = 0;
		stats_loaded = false;
		exitWith {};
	};
